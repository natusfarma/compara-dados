package com.natusfarma.pc.itecvstotvs.componente.financeiro.cheque.primario;
...
public class PriCheque extends Databases<ModeloCheque> {
    ...
    public List<ModeloCheque> processar(LocalDate inicio, LocalDate fim){
        String condicao = montaCondicaoPeriodo("CH.DT_EMI", inicio, fim );
        String query = lerArquivoConsulta(file, condicao);
        return consulta(query, primaryJdbcTemplate, (rs, rowNum) -> rowMapperCheque.modeloPadrao(rs));
    }
